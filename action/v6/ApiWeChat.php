...
class ApiWeChat extends \action\RestfulApi {
    ...
    function getWeChatInfo() {
        try {
            LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($this->get) . "", "DEBUG");
            $this->beforeWeb();
        } catch (Exception $ex) {
            TigerDAL\CatchDAL::markError(code::$code[code::HOME_INDEX], code::HOME_INDEX, json_encode($ex));
        }
    ...
    public function beforeWeb() {
        ...
        if (empty($this->header['openid'])) {                             //如果$_SESSION中没有openid，说明用户刚刚登陆，就执行getCode、getOpenId、getUserInfo获取他的信息  
            $this->code = $this->getCode();
            LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($this->code) . "", "DEBUG");
            ...
            LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($this->access_token) . "", "DEBUG");
            $userInfo = $this->getUserInfo();
            LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($userInfo) . "", "DEBUG");
            if (!empty($userInfo) && !empty($userInfo['openid'])) {
                ...
                LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($result) . "", "DEBUG");
                if (empty($result)) {
                    // 插入一条用户表记录
                    $_user_info=[
                        'name' => 'wechatMute',
                    ...
                    $_user_id=$UserInfoDAL->insert_id($_user_info);
                    // 插入微信关联表记录
                    $_data = [
                    ...
                    LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($_data) . "", "DEBUG");
    ...
    public function beforeWebMute() {
        ...
        if (empty($this->header['openid'])) {                             //如果$_SESSION中没有openid，说明用户刚刚登陆，就执行getCode、getOpenId、getUserInfo获取他的信息  
            ...
            LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($this->access_token) . "", "DEBUG");
            $result = $wechat->getOpenId($this->access_token['openid']);     //根据OPENID查找数据库中是否有这个用户，如果没有就写数据库。继承该类的其他类，用户都写入了数据库中。  
            LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($result) . "", "DEBUG");
            if (empty($result)) {
                // 插入一条用户表记录
                $_user_info=[
    ...
    public function getToken() {
        ...
        LogDAL::saveLog("DEBUG", "INFO", $userinfo_url);
        $userinfo_json = $this->https_request($userinfo_url);
        LogDAL::save(date("Y-m-d H:i:s") . "----" . json_encode($userinfo_json) . "", "DEBUG");
        $userinfo_array = json_decode($userinfo_json, TRUE);
        return $userinfo_array;
    }
